...
class TrainTask(BaseTask):
    ...
    def _prepare(self):
        ...
        # load state dict
        backbone.load_state_dict(torch.load(self.cfg['BACKBONE_RESUME']), False)
        # state_dict_saved = torch.load(self.cfg['BACKBONE_RESUME'])
        # new_state_dict = {}
        # for key in state_dict_saved:
        #     if key.startswith('output_layer.3.'):
        #         new_state_dict[key.replace('output_layer.3.', 'output_layer.5.')] = state_dict_saved[key]
        #     elif key.startswith('output_layer.4.'):
        #         new_state_dict[key.replace('output_layer.4.', 'output_layer.6.')] = state_dict_saved[key]
        #     else:
        #         new_state_dict[key] = state_dict_saved[key]
        # backbone.load_state_dict(new_state_dict, False)
        # del new_state_dict, state_dict_saved
...
def main():
    ...
    parser.add_argument('--config', default='thirdparty_pkgs/TFace/config/finetuning.yaml')
