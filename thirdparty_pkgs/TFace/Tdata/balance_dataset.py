...
from Tdata.datasets import FaceDataset
from Tdata.index_tfr_dataset import IndexTFRDataset
from torch.utils.data.sampler import Sampler


class IDBalanceDataset(FaceDataset):
    """ IDBalanceDataset, generates batch Tdata which each class contains fixed num samples.
    """

    def __init__(self, root_dir, index_file, transform):
        """ Create a ``IDBalanceDataset`` object
            Args:
                root_dir: image Tdata dir
...
class IDBalanceTFRDataset(IndexTFRDataset):
    """ IDBalanceDataset for TFRDataset, generates batch Tdata which each class contains fixed num samples.
    """
