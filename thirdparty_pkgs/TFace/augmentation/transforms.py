...
class CutoutOp(object):
    ...
    def __call__(self, img):
        ...
        Returns:
            Tensor: Image with n_holes of dimension length xs length cut out of it.
        """
        w, h = img.size
...
class Addmask(BaseTransform):
    def __init__(self, prob, mag):
        super(Addmask, self).__init__(prob, mag)
        self.AddmaskOp = AddmaskOp(block_scale=self.mag)

    def transform(self, img):
        return self.AddmaskOp(img)


class AddmaskOp(object):
    def __init__(self, block_scale):
        self.block_scale = block_scale
        self.diffw = -2  # 5/256*112
        self.diffh = int(30 * self.block_scale)
        self.glass_masks_dir = os.path.join(os.path.dirname(__file__), './masks_mask')
        self.glass_mats = []
        self.face_5points = [[40 + self.diffw, 80 + self.diffh], [72 - self.diffw, 80 + self.diffh],
                             [56, 62]]  # 112*112
...
class Addglass(BaseTransform):
    def __init__(self, prob, mag):
        super(Addglass, self).__init__(prob, mag)
        self.Addglass_op = AddglassOp()

    def transform(self, img):
        return self.Addglass_op(img)


class AddglassOp(object):
    def __init__(self):
        self.masks_dir = os.path.join(os.path.dirname(__file__), './glass_mask')
        self.glass_mats = []
...
if __name__ == '__main__':
    img = Image.open("/apdcephfs/share_430409/facedata/faces_glintasia/trainconverted_data/0.0/0.0_0.jpg")
    ...
    for _ in range(10):
        tran(img).save('./test_{}.jpg'.format(_))
