...
def setup_training_loop_kwargs(
        ...
        # Dataset.
        data=None,  # Training dataset (required): <data_path>
        cond=None,  # Train conditional model based on dataset labels: <bool>, default = False
        subset=None,  # Train with only N images: <int>, default = all
        mirror=None,  # Augment dataset with xs-flips: <bool>, default = False
    ...
    # -----------------------------------
    # Dataset: Tdata, cond, subset, mirror
    # -----------------------------------
    ...
    try:
        training_set = networks.stylegan2_ada_pytorch.dnnlib.util.construct_class_by_name(
    ...
    args.G_kwargs = dnnlib.EasyDict(class_name='training.thirdparty_pkgs.Generator', z_dim=512, w_dim=512,
                                    mapping_kwargs=dnnlib.EasyDict(), synthesis_kwargs=dnnlib.EasyDict())
    args.D_kwargs = dnnlib.EasyDict(class_name='training.thirdparty_pkgs.Discriminator', block_kwargs=dnnlib.EasyDict(),
    ...
    if cfg == 'cifar':
        args.loss_kwargs.pl_weight = 0  # disable data_path length regularization
    ...
    if resume is None:
        ...
        desc += '-resumecustom'
        args.resume_pkl = resume  # custom data_path or url
...
def subprocess_fn(rank, args, temp_dir):
    networks.stylegan2_ada_pytorch.dnnlib.util.Logger(file_name=os.path.join(args.run_dir, 'log.txt'), file_mode='a',
                                                      should_flush=True)
...
# Dataset.
@click.option('--Tdata', help='Training Tdata (directory or zip)', metavar='PATH', required=True)
@click.option('--cond', help='Train conditional model based on dataset labels [default: false]', type=bool,
              metavar='BOOL')
@click.option('--subset', help='Train with only N images [default: all]', type=int, metavar='INT')
@click.option('--mirror', help='Enable dataset xs-flips [default: false]', type=bool, metavar='BOOL')
# Base config.
...
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    python train.py --outdir=~/training-runs --Tdata=~/mydataset.zip --gpus=1

    \b
    # Train class-conditional CIFAR-10 using 2 GPUs.
    python train.py --outdir=~/training-runs --Tdata=~/datasets/cifar10.zip \\
        --gpus=2 --cfg=cifar --cond=1

    \b
    # Transfer learn MetFaces from FFHQ using 4 GPUs.
    python train.py --outdir=~/training-runs --Tdata=~/datasets/metfaces.zip \\
        --gpus=4 --cfg=paper1024 --mirror=1 --resume=ffhq1024 --snap=10

    \b
    # Reproduce original StyleGAN2 config F.
    python train.py --outdir=~/training-runs --Tdata=~/datasets/ffhq.zip \\
    ...
    \b
    Transfer learning source thirdparty_pkgs (--resume):
    ...
    """
    networks.stylegan2_ada_pytorch.dnnlib.util.Logger(should_flush=True)
    ...
    print(f'Training Tdata:      {args.training_set_kwargs.data_path}')
    print(f'Training duration:  {args.total_kimg} kimg')
    print(f'Number of GPUs:     {args.num_gpus}')
    print(f'Number of images:   {args.training_set_kwargs.max_size}')
    print(f'Image resolution:   {args.training_set_kwargs.resolution}')
    print(f'Conditional model:  {args.training_set_kwargs.use_labels}')
    print(f'Dataset xs-flips:    {args.training_set_kwargs.xflip}')
    print()
