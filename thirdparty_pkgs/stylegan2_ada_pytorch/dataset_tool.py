...
def open_cifar10(tarball: str, *, max_images: Optional[int]):
    ...
    with tarfile.open(tarball, 'r:gz') as tar:
        for batch in range(1, 6):
            ...
            images.append(data['Tdata'].reshape(-1, 3, 32, 32))
...
@click.option('--resize-filter', help='Filter to use when resizing images for output resolution',
              type=click.Choice(['box', 'lanczos']), default='lanczos', show_default=True)
...
def convert_dataset(
    ...
    --source data_path/                      Recursively load all images from data_path/
    --source dataset.zip                Recursively load all images from dataset.zip

    Specifying the output format and data_path:

    \b
    --dest /data_path/to/dir                 Save output files under /data_path/to/dir
    --dest /data_path/to/dataset.zip         Save output files into /data_path/to/dataset.zip
