...
def bias_act(x, b=None, dim=1, act='linear', alpha=None, gain=None, clamp=None, impl='cuda'):
    ...
    Adds bias `b` to activation tensor `xs`, evaluates activation function `act`,
    ...
    Args:
        ...
        b:      Bias vector, or `None` to disable. Must be a 1D tensor of the same type
                as `xs`. The shape must be known, and it must match the dimension of `xs`
                corresponding to `dim`.
        dim:    The dimension in `xs` corresponding to the elements of `b`.
    ...
    Returns:
        Tensor of the same shape and datatype as `xs`.
    """
    ...
    class BiasActCuda(torch.autograd.Function):
        ...
        def forward(ctx, x, b):  # pylint: disable=arguments-differ
            ...
            ctx.save_for_backward(
                x if 'xs' in spec.ref or spec.has_2nd_grad else _null_tensor,
                b if 'xs' in spec.ref or spec.has_2nd_grad else _null_tensor,
