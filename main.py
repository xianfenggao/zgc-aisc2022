...
parser.add_argument('-dp', '--data_path', type=str, default='dataset/Face/zgc-aisc2022/data')
...
class zgc_aiscPairs(Dataset):
    def __init__(self, trans=None, eval_nums=3000):
    ...
    def __getitem__(self, item):
        item += 1
        source_file = os.path.join(args.data_path, "%04d.png"%item)
        source_img = Image.open(source_file)

        target_file = os.path.join(args.data_path, "%04d_compare.png"%item)
...
def main():
    ...
    data = zgc_aiscPairs(trans=trans.ToTensor(), eval_nums=args.adv_nums)
    LFWLoader = DataLoader(data, batch_size=args.batch_size)
    ...
    for i, (source_imgs, target_imgs) in enumerate(tqdm(LFWLoader)):
        
        if os.path.exists(os.path.join(cur_output_dir, "%04d.png"%(args.batch_size*(i+1)))):
            log_file.print('Batch {}/{} already exists, skip...'.format(i+1, len(LFWLoader)))
            continue

        log_file.print(
            "\n*" * 5 + 'pair %03d' % i + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())) + "*" * 10)
        ...
        for k in range(1, 1+args.num_iter):
            ...
            if args.train_with_eval:
                ...
                # print(eval_score_y)
            plt.grid()
            plt.savefig(os.path.join(output_dir, 'loss-batch_%03d.png' % i))
            plt.close()
        ...
        # _canvas = canvas.clone().detach().cpu().numpy().transpose([0, 2, 3, 1]).astype('uint8')
        ...
        # targets = target_imgs.clone().detach().cpu().numpy().transpose([0, 2, 3, 1]).astype('uint8')
        # sources = source_imgs.clone().detach().cpu().numpy().transpose([0, 2, 3, 1]).astype('uint8')
        for j in range(args.batch_size):
            # save result
            cur_id = args.batch_size * i + j + 1
            # Image.fromarray(_canvas[j]).save(os.path.join(cur_output_dir, "output_face.png"))
            Image.fromarray(image_aligned[j]).save(os.path.join(cur_output_dir, "%04d.png"%(cur_id)))
            # Image.fromarray(canvas[j]).save(os.path.join(cur_output_dir, 'canvas.png'))
            # Image.fromarray(sources[j]).save(os.path.join(cur_output_dir, 'source.png'))
            # Image.fromarray(targets[j]).save(os.path.join(cur_output_dir, 'target.png'))
